		...
		class CWW_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' 		=> esc_html__('Getting Started', 'xews-lite'),
					'recommended_plugins' 	=> esc_html__('Recommended Plugins', 'xews-lite'),
					'demo_import' 			=> esc_html__('Import Demo', 'xews-lite'),
					'changelogs'			=> esc_html__('Changelogs','xews-lite'),
					'support' 				=> esc_html__('Support', 'xews-lite'),
			...
			<h3>
			<?php
			printf( '%1$s %2$s !! %3$s <a href="%4$s">%5$s</a> %6$s', esc_html__( 'Welcome! Thank you for choosing', 'xews-lite' ), esc_html($this->theme_name), esc_html__( 'Please make sure you visit our', 'xews-lite' ), esc_url( admin_url( 'themes.php?page=welcome-page' ) ), esc_html__( 'Welcome Page', 'xews-lite' ), esc_html__( 'to get started.', 'xews-lite' ) );
			?>
			</h3>
			<p>

				<?php if( class_exists('CWW_Companion')){ ?>
					<a class="button button-primary button-hero cww-action-btn" href="<?php echo esc_url(admin_url( 'themes.php?page=welcome-page' )) ?>"><?php esc_html_e( 'Visit Welcome Page', 'xews-lite' ); ?></a>
				<?php }else{ ?>
					<a class="button button-primary button-hero cww-action-btn btn-companion-install" data-file="cww-companion.php" data-slug="cww-companion" href="<?php echo esc_url(admin_url( 'themes.php?page=welcome-page' )) ?>"><?php esc_html_e( 'Lets Get Started', 'xews-lite' ); ?></a>
				<?php } ?>
				
			</p>
			<?php if(! class_exists('CWW_Companion')){ ?>
				<p class="msg-blured"><?php esc_html_e('Clicking the button will install & activate CWW Companion plugin.','xews-lite');?></p>
			<?php } ?>
			<span class="dismiss"><a href="?theme-info-ignore-notice"><?php esc_html_e('Never show this info','xews-lite') ?></a></span>
			</div>
			...
			public function welcome_styles_and_scripts() {
				wp_enqueue_style( 'xews-lite' . '-welcome-screen', get_template_directory_uri() . '/inc/welcome/css/welcome.css' );
				wp_enqueue_script( 'xews-lite' . '-welcome-screen', get_template_directory_uri() . '/inc/welcome/js/welcome.js', array( 'jquery' ) );

				wp_localize_script( 'xews-lite' . '-welcome-screen', 'CWWWelcomeObject', array(
			...
			public function generate_plugin_url($status, $plugin) {
				...
				if ( ! current_user_can('install_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'xews-lite' ) );
				}
			...
			public function plugin_installer_callback(){
				...
				if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'xews-lite') );
				}
			...
			public function plugin_activation_callback() {
				...
				if ( ! current_user_can('activate_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to activate plugins on this site.', 'xews-lite' ) );
				}
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'xews-lite');
				}
			...
			public function plugin_deactivation_callback() {
				...
				if ( ! current_user_can('deactivate_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to deactive plugins on this site.', 'xews-lite' ) );
				}
	      	...
	      	public function admin_sidebar_contents(){ ?>
	      		<div class="cww-admin-side-wrapper">
					<div class="cww-side-box-wrapper cww-welcome-box-white">
						<div class="box-header cww-box-header"><?php esc_html_e('Theme Documentation','xews-lite'); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('We have online documentation to make your configuration process easy, go through the documentation to get more familiar with theme','xews-lite'); ?></p>
							<a href="<?php echo esc_url($this->strings['doc_link']);?>" class="button button-primary" target="_blank"><?php esc_html_e('Read Now','xews-lite'); ?></a>
						</div>
					</div>
